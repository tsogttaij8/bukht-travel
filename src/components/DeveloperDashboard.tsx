...
export default function DeveloperDashboard({ currentRoles }: DeveloperDashboardProps) {
  ...
  const { data, setData, loading, loadError } = useDashboardData({ isOwner, canManageProducts, canManageShipments })
  ...
  return (
    <div className="developer-dashboard" style={{ display: "grid", gap: 20 }}>
      ...
      <DashboardStats isOwner={isOwner} canManageProducts={canManageProducts} canManageShipments={canManageShipments} totalStaff={staff.totalStaff} data={data} />
