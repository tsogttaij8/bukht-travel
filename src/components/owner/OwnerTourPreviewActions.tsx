...
export default function OwnerTourPreviewActions({ tourId, status }: { tourId: string; status: TravelPackageStatus }) {
  ...
  async function updateStatus(nextStatus: TravelPackageStatus): Promise<void> {
    ...
    if (!response.ok) {
      ...
      setError(body.message ?? "Failed to update tour status.")
      return
    }
    router.refresh()
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <Link href={`/owner/travel/tours/${encodeURIComponent(tourId)}`} className="rounded-md border border-blue-200 bg-white px-3 py-2 text-sm font-black text-blue-900 hover:bg-blue-50">Back to edit</Link>
